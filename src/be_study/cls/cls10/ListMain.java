...
public class ListMain {
	...
	public static void main(String[] args) {
		...
		//ArrayList
		//ArrayList <저장타입>저장타입 클래스 타입(참조변수)가능
		ArrayList <String> a1 = new ArrayList<String>();
		//ArrayList <int> a12 = new ArrayList<int>();//기본타입 불가 int double boolean
		ArrayList<Integer>a12 = new ArrayList<Integer>();
//		
	//	Wrapper Class(기본타입을 한번 감싸서 (포장) 객체참도타입으로 인식되도록 하는 클래스)
...
//		boolean.parseBoolean(null)  boolean
		...
		//기본배열
		String[]sArr1=new String[4];//4칸짜리 배열공간확보
		...
		//ArrayList
		//ArrayList <저장타입> 변수명= new ArrayList <저장할 타입>();
		
		//List< 저장타입>변수명=new ArrayList <>();
		
		ArrayList<String> sList =new ArrayList <String>();
		
		sList.add("A"); //add() List에 값추가
		sList.add("B");
		sList.add("C");
		sList.add("D");
		for (String s: sList) {
		...
		//List 에서 길이는list.size(  )   | array.length
		for (int i=0; i<sList.size(); i++) {
			//List 에서 특정 인덱스 위치의 값어 접근.   .get(index)    arr[index]
		...
		//List 에서 값삭제 remove(index)  remove(값)   //arr[i]=null
